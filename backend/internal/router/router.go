...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	invalidationRepository := repository.NewInvalidationRepository(db)
	...
	emissionSampleService := service.NewEmissionSampleService(emissionSampleRepository, complianceDecisionRepository, invalidationRepository, securityService)
	complianceDecisionService := service.NewComplianceDecisionService(complianceDecisionRepository, emissionSampleService, securityService)
