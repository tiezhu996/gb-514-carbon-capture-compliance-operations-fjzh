...
export function statusTone(status: string): 'success' | 'warning' | 'danger' | 'neutral' {
  if (/approved|accepted|released|completed|signed|closed|pass|ready|online|cleared|succeeded|verified/.test(status)) return 'success';
  if (/failed|rejected|critical|scrap|discard|revoked|urgent|invalid/.test(status)) return 'danger';
